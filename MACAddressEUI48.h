...
//-----------------------------------------------------------------
#pragma once
#ifndef MAC_ADDRESS_EUII48_H
#define MAC_ADDRESS_EUII48_H
#include <Arduino.h>
#include "MACAddressEUI64.h"

#define MAC_EUI48_FORMAT_UPPER_COLON "%02X:%02X:%02X:%02X:%02X:%02X"
#define MAC_EUI48_FORMAT_LOWER_COLON "%02x:%02x:%02x:%02x:%02x:%02x"
#define MAC_EUI48_FORMAT_UPPER_HYPHEN "%02X-%02X-%02X-%02X-%02X-%02X"
#define MAC_EUI48_FORMAT_LOWER_HYPHEN "%02x-%02x-%02x-%02x-%02x-%02x"
#define MAC_EUI48_FORMAT_UPPER_PERIOD "%04X.%04X.%04X"
#define MAC_EUI48_FORMAT_LOWER_PERIOD "%04x.%04x.%04x"

#define MAC_EUI48_BYTES_LENGTH 6
#define MAC_EUI48_WORDS_LENGTH 3
#define MAC_EUI48_STR_LENGTH 32


class MACAddressEUI48 : public Printable {
public:

private:
  union {
    uint8_t bytes[MAC_EUI48_BYTES_LENGTH];
    uint16_t words[MAC_EUI48_WORDS_LENGTH];
  } _address;
...
public:
  ...
  MACAddressEUI48(const MacFormat format) {
    clear();
    _format = format;
  }
  MACAddressEUI48(const uint8_t firstOctet, const uint8_t secondOctet, const uint8_t thirdOctet, const uint8_t fourthOctet, const uint8_t fifthOctet, const uint8_t sixthOctet) {
    fromBytes(firstOctet, secondOctet, thirdOctet, fourthOctet, fifthOctet, sixthOctet);
    _format = _defaultFormat;
  }
  MACAddressEUI48(const uint8_t firstOctet, const uint8_t secondOctet, const uint8_t thirdOctet, const uint8_t fourthOctet, const uint8_t fifthOctet, const uint8_t sixthOctet, const MacFormat format) {
    fromBytes(firstOctet, secondOctet, thirdOctet, fourthOctet, fifthOctet, sixthOctet);
    _format = format;
  }
  MACAddressEUI48(const uint16_t firstWord, const uint16_t secondWord, const uint16_t thirdWord) {
    fromWords(firstWord, secondWord, thirdWord);
    _format = _defaultFormat;
  }
  MACAddressEUI48(const uint16_t firstWord, const uint16_t secondWord, const uint16_t thirdWord, const MacFormat format) {
    fromWords(firstWord, secondWord, thirdWord);
    _format = format;
  }
  MACAddressEUI48(const uint8_t* bytes) {
    fromBytes(bytes);
    _format = _defaultFormat;
  }
  MACAddressEUI48(const uint8_t* bytes, const MacFormat format) {
    fromBytes(bytes);
    _format = format;
  }
  MACAddressEUI48(const uint16_t* words) {
    fromWords(words);
    _format = _defaultFormat;
  }
  MACAddressEUI48(const uint16_t* words, const MacFormat format) {
    fromWords(words);
    _format = format;
  }
  MACAddressEUI48(const MACAddressEUI48& addr) {
    fromBytes(addr._address.bytes);
    _format = _defaultFormat;
  }
  MACAddressEUI48(const MACAddressEUI48& addr, const MacFormat format) {
    fromBytes(addr._address.bytes);
    _format = format;
  }
  MACAddressEUI48(const char* addr) {
    fromString(addr);
    _format = _defaultFormat;
  }
  MACAddressEUI48(const char* addr, const MacFormat format) {
    fromString(addr);
    _format = format;
  }
  MACAddressEUI48(const String& addr) {
    fromString(addr);
    _format = _defaultFormat;
  }
  MACAddressEUI48(const String& addr, const MacFormat format) {
    fromString(addr);
    _format = format;
  }
  ...
  bool fromString(const String& addr, const MacFormat format) {
    char str[MAC_EUI48_STR_LENGTH];
    addr.toCharArray(str, MAC_EUI48_STR_LENGTH);
  ...
  static bool fromString(uint8_t* bytes, const char* addr) {
    uint8_t newBytes[MAC_EUI48_BYTES_LENGTH];

    int result = 0;
    result = sscanf(addr, MAC_EUI48_FORMAT_UPPER_COLON, &newBytes[0], &newBytes[1], &newBytes[2], &newBytes[3], &newBytes[4], &newBytes[5]);
    if (result == MAC_EUI48_BYTES_LENGTH) {
      fromBytes(bytes, newBytes);
      return true;
    }
    result = sscanf(addr, MAC_EUI48_FORMAT_LOWER_COLON, &newBytes[0], &newBytes[1], &newBytes[2], &newBytes[3], &newBytes[4], &newBytes[5]);
    if (result == MAC_EUI48_BYTES_LENGTH) {
      fromBytes(bytes, newBytes);
      return true;
    }

    result = sscanf(addr, MAC_EUI48_FORMAT_UPPER_HYPHEN, &newBytes[0], &newBytes[1], &newBytes[2], &newBytes[3], &newBytes[4], &newBytes[5]);
    if (result == MAC_EUI48_BYTES_LENGTH) {
      fromBytes(bytes, newBytes);
      return true;
    }
    result = sscanf(addr, MAC_EUI48_FORMAT_LOWER_HYPHEN, &newBytes[0], &newBytes[1], &newBytes[2], &newBytes[3], &newBytes[4], &newBytes[5]);
    if (result == MAC_EUI48_BYTES_LENGTH) {
      fromBytes(bytes, newBytes);
      return true;
    }
  ...
  static bool fromString(uint16_t* words, const char* addr) {
    uint16_t newWords[MAC_EUI48_WORDS_LENGTH];

    int result = 0;
    result = sscanf(addr, MAC_EUI48_FORMAT_UPPER_PERIOD, &newWords[0], &newWords[1], &newWords[2]);
    if (result == MAC_EUI48_WORDS_LENGTH) {
      fromWords(words, newWords);
      return true;
    }
    result = sscanf(addr, MAC_EUI48_FORMAT_LOWER_PERIOD, &newWords[0], &newWords[1], &newWords[2]);
    if (result == MAC_EUI48_WORDS_LENGTH) {
      fromWords(words, newWords);
      return true;
    }
  ...
  static bool fromString(uint16_t* words, const char* addr, const MacFormat format) {
    uint16_t newWords[MAC_EUI48_BYTES_LENGTH];
  ...
  static void fromBytes(uint8_t* target, const uint8_t* source) {
    memcpy(target, source, MAC_EUI48_BYTES_LENGTH);
  }
  ...
  static void fromWords(uint16_t* target, const uint16_t* source) {
    memcpy(target, source, MAC_EUI48_WORDS_LENGTH);
  }

  bool equals(const MACAddressEUI48& addr) const {
  ...
  bool equals(const uint8_t* bytes) const {
    return equals(bytes[0], bytes[1], bytes[2], bytes[3], bytes[4], bytes[5]);
  }
  bool equals(const uint16_t* words) const {
    return equals(words[0], words[1], words[2]);
  }
  ...
  bool operator==(const MACAddressEUI48& addr) const {
    return equals(addr);
  }
  bool operator!=(const MACAddressEUI48& addr) const {
    return !equals(addr);
  ...
  MACAddressEUI48& operator=(const uint8_t* bytes) {
    fromBytes(bytes);
    return *this;
  }
  MACAddressEUI48& operator=(const char* addr) {
    fromString(addr);
    return *this;
  }
  MACAddressEUI48& operator=(const String& addr) {
    fromString(addr);
    return *this;
  }
  ...
  static int toCharArray(char* c_array, const uint8_t* bytes, const MacFormat format) {
    switch(format){
      case MacFormat::UPPER_COLON:
        return sprintf(c_array, MAC_EUI48_FORMAT_UPPER_COLON, bytes[0], bytes[1], bytes[2], bytes[3], bytes[4], bytes[5]);
        
      case MacFormat::LOWER_COLON:
        return sprintf(c_array, MAC_EUI48_FORMAT_LOWER_COLON, bytes[0], bytes[1], bytes[2], bytes[3], bytes[4], bytes[5]);
        
      case MacFormat::UPPER_HYPHEN:
        return sprintf(c_array, MAC_EUI48_FORMAT_UPPER_HYPHEN, bytes[0], bytes[1], bytes[2], bytes[3], bytes[4], bytes[5]);
        
      case MacFormat::LOWER_HYPHEN:
        return sprintf(c_array, MAC_EUI48_FORMAT_LOWER_HYPHEN, bytes[0], bytes[1], bytes[2], bytes[3], bytes[4], bytes[5]);
    }
    return -1;
  }
  ...
  static int toCharArray(char* c_array, const uint16_t* words, const MacFormat format) {
    switch(format){
      case MacFormat::UPPER_PERIOD:
        return sprintf(c_array, MAC_EUI48_FORMAT_UPPER_PERIOD, words[0], words[1], words[2]);
        
      case MacFormat::LOWER_PERIOD:
        return sprintf(c_array, MAC_EUI48_FORMAT_LOWER_PERIOD, words[0], words[1], words[2]);
    }
    return -1;
  }
